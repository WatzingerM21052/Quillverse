...
**Merged into `main` 2026-08-21** after the user confirmed the concurrent picture-library session had
finished and pushed. Branch `worktree-prompt-verbessern` and its scratch worktree (including the SDD
ledger) were removed after the merge, per `superpowers:subagent-driven-development`'s finish step — this
worklog entry and the commit history are the lasting record.
